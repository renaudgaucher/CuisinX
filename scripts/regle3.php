...
if (!ctype_digit($id_recette) || !ctype_digit($nb_personne)) return false;
    ...
    foreach ($recette->liste_ingredient as $ingredient) {
        echo "<li>".round(($ingredient->quantite *$nb_personne/$nb_personne_theorique),2)." ".htmlspecialchars("$ingredient->unite"). " " . htmlspecialchars(Ingredient::id_to_nomIngredient($dbh, $ingredient->id_ingredient)['nom']) . "</li>";
    }
    $dbh=null;
}
